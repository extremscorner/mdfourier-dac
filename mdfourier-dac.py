...
video game consoles.
 ...
 1.0 s sync sequence (1/3 s silent, 1/3 s 8K sines, 1/3 s silent)
...
10.0 s 1/8 pulse sweep from 20 Hz to 20480 Hz at 2 seconds per octave,
 ...
 1.2 s padding
 4.0 s 50% pulse fading from full scale to nothing, with amplitude as quadratic function of time
 1.0 s sync sequence
...
# Two rates to use are 44100, which produces 588-sample sectors,
# and 48000, which produces 640-sample sectors
fps = 75
...
def get_fade(length, halfperiod):
    ...
    fade = fade * fade
    square = np.full((halfperiod,), 24576)
